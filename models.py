...
class Participant(models.Model):
    ...
    # photo = models.ImageField(upload_to=UploadToPathAndRename('FPT2019/id_photo'), help_text="Merci d'utiliser une photo d'identité.", null=True)
    ...
    # Ce code devrait rendre automatique le tri des listes de participants. TODO : le tester
    #class Meta:
...
class Team(models.Model):
    ...
    IOC = models.OneToOneField(User, on_delete=models.SET_NULL, null=True,
                               blank=True, related_name='Team_FPT2019', verbose_name="Référent")
    ...
    def problems(self, verbose=False, currentround=None):
        ...
        rounds = Round.objects.filter(reporter_team=self)
        ...
        rounds = Round.objects.filter(opponent_team=self)
        rounds = [round for round in rounds if round.pf_number < pf_number]
        opposed = []
        for round in rounds:
            if verbose:
...
class Jury(models.Model):
    ...
    email = models.EmailField(
        help_text='Cette adresse sera utilisée pour envoyer des informations importantes aux jurés.', verbose_name='Email', null=True)
...
class Round(models.Model):
    ...
    round_number = models.IntegerField(
        choices=(((ind + 1, 'Round ' + str(ind + 1)) for ind in range(2))),
        default=None
    )
...
def update_points(sender, instance, **kwargs):
    print "YOLOOOO !!!"
    if (instance.reporter_team is None) or (instance.opponent_team is None) or instance.problem_presented is None:
        # then all teams aren't yet defined, there is no need to compute scores
        pass
    else:
        teams = [instance.reporter_team, instance.opponent_team]
...
def addTLtoJury():
    teamleaders = Participant.objects.filter(role='TL')
    jurys = Jury.objects.all()

    def alreadyInJurys(teamleader):
        for jury in jurys:
            if jury.name == teamleader.name and jury.surname == teamleader.surname:
                return True
        return False

    for tl in teamleaders:
        if not alreadyInJurys(tl):
            Jury.objects.create(surname=tl.surname, name=tl.name, affiliation=str(tl.team), team=tl.team, pf1=True, pf2=True, pf3=True, pf4=True)
